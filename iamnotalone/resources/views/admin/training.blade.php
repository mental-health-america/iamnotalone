...
@section('content')
                ...
                @if (count($trainings))
                        @foreach ($trainings as $training)
                            <tr class="bg-white my-3 text-center">
                                ...
                                <td>
                                    @if (strtotime($training->start_date." ".$training->start_time) >= strtotime(date('Y-m-d h:i:s')) && strtotime($training->end_date." ".$training->end_time) >= strtotime(date('Y-m-d h:i:s')))
                                        <span class="text-white bg-green-300 rounded-full py-2 px-4">Ongoing</span>
                                    @elseif(strtotime($training->start_date." ".$training->start_time) >= strtotime(date('Y-m-d h:i:s')) && strtotime($training->end_date." ".$training->end_time) < strtotime(date('Y-m-d h:i:s')))
                                ...
                                <td>
                                    <div class="relative">
                                        <a href="#" class="relative z-2 block rounded-md bg-white p-2 focus:outline-none" onclick="showHideMenu('op{{$training->id}}')">
                                        ...
                                        <div class="absolute right-0 mt-2 ml-10 w-32 bg-white rounded-md shadow-xl z-20 hidden" id="op{{$training->id}}">
                                            <a href="{{route('admin.training.details', ['id'=>$training->id])}}" class="block px-2 py-3 rounded text-sm capitalize text-gray-700 bg-gradient-to-r hover:from-purple-400 hover:via-pink-500 hover:to-red-500 hover: hover:text-white">
                                                Edit Training
                                            </a>
                                            <a href="{{route('admin.training.remove', ['id'=>$training->id])}}" class="block px-2 py-3 rounded text-sm capitalize text-gray-700 bg-gradient-to-r hover:from-purple-400 hover:via-pink-500 hover:to-red-500 hover: hover:text-white">
                                                Delete Training
                                            </a>
                                        </div>
                                    </div>
    ...
    <div id="trainingModal" class="modal" style="width: 75%; max-width: 75%; padding: 3% 10%">
        <form class="text-center" method="POST" action="{{route('admin.training.new')}}" enctype="multipart/form-data">
            @csrf
