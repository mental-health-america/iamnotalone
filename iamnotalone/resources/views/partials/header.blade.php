<nav class="flex items-center flex-wrap mb-10 mt-2">
    <a href="/" class="p-2 mx-4 inline-flex items-center">
      ...
      <span class="text-xl text-white font-bold uppercase tracking-wide">Talwind CSS</span>-->
        <img class="block h-14 md:h-20 w-auto" src="{{asset('images/logo.png')}}" alt="HMS Logo">
    </a>
    ...
    <div class="hidden top-navbar w-full lg:inline-flex lg:flex-grow lg:w-auto" id="navigation">
        <div class="lg:inline-flex lg:flex-row lg:ml-auto lg:w-auto w-full lg:items-center items-start px-2 py-4 flex flex-col lg:h-auto bg-gray-900 md:bg-white lg:mr-6">
          ...
          <a id="events" href="{{route('event.new')}}" class="lg:inline-flex lg:w-auto w-full px-4 py-2 rounded text-white md:text-black text-sm font-medium uppercase items-center justify-center hover:bg-indigo-500 hover:text-white">
          ...
          <div @click.away="open = false" class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute z-10 left-0.5 right-0 w-36 mt-2 ml-2 origin-top-right rounded-md shadow-lg md:w-48">
              <div class="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                <a class="block text-center px-2 py-2 mt-2 text-sm  bg-transparent rounded-lg  hover:text-white focus:text-gray-900 hover:bg-indigo-400 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="{{route('forum')}}">General Forum</a>
                <a class="block text-center px-2 py-2 mt-2 text-sm  bg-transparent rounded-lg  hover:text-white focus:text-gray-900 hover:bg-indigo-400 focus:bg-gray-200 focus:outline-none focus:shadow-outline" href="{{route('peers.forum')}}">Peers Forum</a>
              </div>
            </div>
          </div>
          <a href="{{route('donate')}}" class="lg:inline-flex bg-yellow-600 hover:bg-yellow-400 md:w-auto w-auto mx-4 my-2 px-4 py-2 rounded text-white text-sm font-medium uppercase items-center justify-center hover:bg-gray-900 hover:text-white">
            <span>Donate</span>
          </a>
        </div>
    </div>
