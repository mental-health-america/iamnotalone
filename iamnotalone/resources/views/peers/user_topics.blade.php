...
@section('content')
    <div class="flex flex-col md:flex-row text-gray-600">
        ...
        <div class="w-full md:w-3/6 flex flex-col px-4 md:px-12">
            ...
            <div class="my-8">
                <h3 class="text-2xl">Topics posted by <span class="text-indigo-800 underline">{{Auth::user()->first_name.' '.Auth::user()->last_name}}</span></h3>
            </div>
                ...
                @foreach ($posts as $post)
                    <div class="w-full mt-8 sm:p-12 p-6 overflow-hidden rounded-lg border-2 shadow-md bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out">
                        <h3 class="mb-4 text-medium">
                            <a href="{{route('peers.forum.post', ['id'=>$post->id])}}">{{$post->title}}</a>
                            <br>
                            <span class="text-sm">Related to <a href="{{route('event.details', ['id'=>$post->event_id])}}" class="text-indigo-800 underline">{{$post->event->name}}</a></span>
                        </h3>
                        ...
                        <div class="flex flex-col sm:flex-row text-xs font-semibold mt-4">
                            ...
                            <a href="{{route('peers.forum.post', ['id'=>$post->id])}}" class="">
                                <span class="sm:w-1/3 mt-2">
                                    {{$post->comments->count()}} <span class="text-indigo-800">Comments</span>
                                </span>
                            </a>
                        </div>
                    </div>
